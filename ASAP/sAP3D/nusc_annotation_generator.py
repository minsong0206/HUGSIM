...
def get_nusc_val_token(nusc, split_mode='trainval'):
    splits = create_splits_scenes()
    if split_mode == 'train':
        val_scene_ids = set(splits['train'])
    elif split_mode == 'val':
        val_scene_ids = set(splits['val'])
    elif split_mode == 'trainval':
        val_scene_ids = set(splits['train']) | set(splits['val'])
    else:
    ...
    for sample in nusc.sample:
        ...
        if scene_name in val_scene_ids:
            sample_val_tokens.append(sample['token'])
    print("total samples in {}: {}".format(split_mode, len(sample_val_tokens)))
...
if __name__ == '__main__':
    ...
    # load 20Hz LiDAR inference results (only needed for 'advanced' strategy)
