...
class HUGSimEnv(gymnasium.Env):
    def __init__(self, cfg, output):
        ...
        _disable_bg = OmegaConf.select(cfg, "base.disable_bg_collision", default=False)
        _scene_mask = ((semantic_idx > 1) & (semantic_idx != 10))[:, 0] if not _disable_bg else torch.zeros(semantic_idx.shape[0], dtype=torch.bool)
        scene_xyz = self.gaussians.get_full_xyz[_scene_mask].detach().cpu().numpy()
        ...
        scene_pts = scene_xyz if len(scene_xyz) > 0 else np.zeros((1, 3), dtype=float)
        scene_pcd.points = o3d.utility.Vector3dVector(scene_pts.astype(float))
        ...
        disable_bg = OmegaConf.select(cfg, "base.disable_bg_collision", default=False)
        self.points = gaussians.get_xyz[torch.zeros(gaussians.get_xyz.shape[0], dtype=torch.bool, device=gaussians.get_xyz.device)] if disable_bg else gaussians.get_xyz[mask]
