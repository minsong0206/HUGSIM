...
def training(cfg):
    ...
    start_ckpt = getattr(cfg, 'start_checkpoint', '')
    if start_ckpt:
        print(f"Resuming from checkpoint: {start_ckpt}")
        (model_params, first_iter) = torch.load(start_ckpt)
        gaussians.restore(model_params, cfg.opt)
...
def main():
    ...
    parser.add_argument("--start_checkpoint", type=str, default="",
                        help="Path to chkpntXXXXX.pth to resume training from that iteration")
    ...
    if len(args.start_checkpoint) > 0:
        cfg.start_checkpoint = args.start_checkpoint
